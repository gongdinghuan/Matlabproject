...
figure;
subplot(1,2,1);imshow(img2);xlabel('标记边界');
hold on 
for k = 1:length(B)
    boundary = B{k};
...
for k =1:length(B)
    boundary = B{k};
    plot(boundary(:,2),boundary(:,1),'w','LineWidth',2); text(boundary(1,2)-11,boundary(1,1)+11,num2str(k),'Color','y','Fontsize',14,'FontWeight','bold'); 
end
...
for k = 1:N 
    scatter(TR(k),areas(k),N,cmap(k,:),'filled'); 
...
for i=1:size(a,1) %图像的行数
    for j=1:size(a,2) %图像的列数
        if a(i,j)>=threshhold %阈值threshhold，灰度值大于阈值的部分。
            sum=sum+1; 
        end
    end
end